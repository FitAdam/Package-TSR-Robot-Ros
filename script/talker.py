...
def predict_model(img_path, model):
    ...
    print(f'It\'s a \"{labels[pred[0]]}\" sign!!')
...
def main():
    file_path = f'/home/adam/Documents/Traffic-Sign-Recognizer/uploads/test.jpeg'
    sign = predict_model(file_path, model)
    return sign


def talker():
    signs_publisher = rospy.Publisher('chatter', String, queue_size=10)
    rospy.init_node('talker', anonymous=True)
    rate = rospy.Rate(10) # 10hz
    while not rospy.is_shutdown():
        new_sign = main()
        sign_str = new_sign
        rospy.loginfo(sign_str)
        signs_publisher.publish(sign_str)
        rate.sleep()
